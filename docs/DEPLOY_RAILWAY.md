...
| `SUPABASE_URL` | **필수(업로드 사용 시)** | Supabase 프로젝트 URL — 어드민 이미지 업로드용 | Supabase → Project Settings → API → **Project URL** |
| `SUPABASE_SERVICE_KEY` | **필수(업로드 사용 시)** | Supabase service_role 키 | Supabase → Project Settings → API → **service_role** (secret) |
| `SUPABASE_STORAGE_BUCKET` | 선택 | 이미지 저장 버킷 이름 | 기본값 `product-images` |
| `NODE_ENV` | 권장 | `production` 지정 시 auth 미들웨어가 시크릿 누락 시 FATAL | 값: `production` |
| `PORT` | 자동 | Railway 가 자동 주입 | - |

> Supabase Storage 관련 세팅(버킷 생성, Public 설정 등) 은 [SUPABASE_STORAGE_SETUP.md](./SUPABASE_STORAGE_SETUP.md) 참고.
