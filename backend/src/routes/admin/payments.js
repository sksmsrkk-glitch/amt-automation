...
router.get('/stats', async (req, res) => {
  try {
    ...
    const totalPayments = await db.prepare('SELECT COUNT(*) as count FROM payments').get().count;
    const totalAmount = await db.prepare("SELECT COALESCE(SUM(amount), 0) as total FROM payments WHERE status = 'paid'").get().total;
    const totalRefunds = await db.prepare("SELECT COALESCE(SUM(refund_amount), 0) as total FROM payments WHERE refund_amount > 0").get().total;
    ...
    const methodBreakdown = await db.prepare(`
    ...
    const todayPayments = await db.prepare("SELECT COUNT(*) as count, COALESCE(SUM(amount), 0) as total FROM payments WHERE DATE(created_at) = ? AND status = 'paid'").get(today);
...
router.get('/', async (req, res) => {
  try {
    ...
    const total = await db.prepare(`SELECT COUNT(*) as count FROM payments p ${whereClause}`).get(...params).count;
...
router.put('/:id/status', async (req, res) => {
  try {
    ...
    await db.prepare("UPDATE bookings SET payment_status = ?, updated_at = datetime('now') WHERE id = ?").run(status, payment.booking_id);

    // 결제가 'paid' 로 확정되면 pending 예약을 confirmed 로 자동 승격.
    if (status === 'paid') {
      await db.prepare("UPDATE bookings SET status = 'confirmed', updated_at = datetime('now') WHERE id = ? AND status = 'pending'").run(payment.booking_id);
    }
