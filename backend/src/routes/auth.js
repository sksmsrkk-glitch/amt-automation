...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = await db.prepare('SELECT id, email, name, phone, nationality, role, language, created_at FROM users WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/me', authenticate, async (req, res) => {
  try {
    ...
    const user = await db.prepare('SELECT id, email, name, phone, nationality, role, language, created_at, updated_at FROM users WHERE id = ?').get(req.user.id);
